...
본 논문에서는 연속적인 latent variable을 효율적으로 inference 하기위해 SGVB로 variational lower bound의 estimation 하는 방법론인 VAE를 제시하였습니다. 이 estimator는 미분 가능하고 SGD로 최적화됩니다. 가우시안 샘플링시 랜덤하게 샘플링하는 방식에서 발견되는 back propagation 에러를 해결하기 위해 reparameterization trick을 사용하여 최적화 합니다. 또한 VAE는 각 datapoint가 연속적인 latent variable를 가지는 i.i.d의 데이터셋과 같은 high dimensional dataset에 대응해 오토인코딩 VB 알고리즘인 SGVB를 사용하여 학습합니다.
